...
![img](https://img-blog.csdnimg.cn/img_convert/fde5e8955b1c0d7366ea094708cd1fc6.png)　　
...
介绍一下TCP IP的协议，三次握手机制

TCP和UDP的区别，哪些适合TCP哪些适合UDP

输入URL后发生了什么

杀死一个叫simulator的进程，用一句通过管道实现：先找到进程的pid号，再 kill-9

常用的排序算法，介绍一个是怎么实现的，介绍快排的原理
...
**介绍下学校课程，有没有学过操作系统？**（简介通信专业课程体系，答学校里没上过课，但是自学过。）
...
**有没有学过数据库？数据库存储方式有哪些类型？了解索引吗？**（答只看过一些网课，后面乱答的，面试官不满意）

**手撕SQL，“在一列数据中，找出存在重复的数据”**（太久没写SQL了，没做出来）

**针对一个用户登录页面的网页，你可以编写那些测试用例？**（说了很多，经面试官提醒又补充了一些，这个回答得还行）

**这个页面从成功登录后，到浏览器正常显示网站内容，中间经历了什么过程？尽可能详细的说一说。**（计算机网络和前端的综合知识，也是老生常谈的问题，DNS解析，TCP/IP，三握四挥，HTML和CSS等串联起来讲，面试官无异议）

**手撕代码，“判断链表中是否存在环”**（蚌埠住了兄弟们，这道题鼠鼠以前做过，但是忙着实习太久没刷力扣，竟然连快慢指针都忘记了，胡言乱语了一通，代码一行没写，感觉面试官也绷不住了）
