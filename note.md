...
- **时间复杂度**：\(O(n)\)，因为递归调用发生了 \(n\) 次。
- **空间复杂度**：\(O(n)\)，递归调用栈的最大深度是 \(n\)。

2. 二叉树深度

- **时间复杂度**：\(O(n)\)，在这里 \(n\) 是树中节点的数量。算法访问每个节点一次。
- **空间复杂度**：最坏情况下（不平衡的树），为 \(O(n)\)；平均情况下（平衡树），为 \(O(log n)\)。

3. 汉诺塔问题

- **时间复杂度**：\(O(2^n)\)，每增加一个盘子，移动次数几乎是之前的两倍。
- **空间复杂度**：\(O(n)\)，由于递归的深度为 \(n\)。

4. 斐波那契数列

使用简单递归方法：

- **时间复杂度**：\(O(2^n)\)，每增加一项，调用次数几乎是之前的两倍。
- **空间复杂度**：\(O(n)\)，递归调用栈的最大深度是 \(n\)。
...
- **时间复杂度**：
  - 平均情况：\(O(n log n)\)
  - 最坏情况：\(O(n^2)\)，当数组已经排序或者全部元素相等时。
- **空间复杂度**：
  - 平均情况：\(O(log n)\)，由于递归调用的深度。
  - 最坏情况：\(O(n)\)，在不平衡的划分情况下。

6. 归并排序

- **时间复杂度**：\(O(n log n)\)，每层递归都需要 \(O(n)\) 的时间来合并子数组，总共有 \(log n\) 层。
- **空间复杂度**：\(O(n)\)，主要是因为合并过程中需要额外的空间。

7. 分治算法

分治算法的时间和空间复杂度取决于特定的问题和实现方式。通常，分治算法将问题分解为若干子问题，递归地解决这些子问题，然后将解合并。复杂度计算通常需要考虑以下因素：

- **子问题的数量**
- **每个子问题解决的复杂度**
- **合并解的复杂度**

例如，在归并排序中，每次将问题分为两个子问题，解决子问题的复杂度是 \(O(log n)\)，合并解的复杂度是 \(O(n)\)。



### 时间复杂度

递归函数的时间复杂度通常根据以下几个因素来评估：

1. **递归调用的次数**：这取决于递归停止的条件。例如，在二分查找中，每次递归都将搜索范围减半，因此时间复杂度是 \(O(log n)\)。

   简单递归求解斐波那契数列（每次产生两个递归调用）的时间复杂度是O(2^n)

2. **每次递归调用的时间消耗**：除了递归调用本身，还需要考虑每次调用中执行的操作量。例如，如果每次递归都进行线性时间的操作，那么总的时间复杂度将是这些操作的复合。

### 空间复杂度

递归的空间复杂度主要由以下因素决定：

1. **递归深度**：递归每深入一层，就会在调用栈上增加额外的空间开销。递归深度直接影响总的空间复杂度。

2. **每次递归调用的空间消耗**：包括局部变量和其他资源的使用。在大多数情况下，这部分空间相对固定，主要影响因素仍是递归的深度。

**示例**

1. **简单的线性递归**：例如计算斐波那契数列的递归实现，每次函数调用只会产生一个递归调用，直到基本情况。这样的递归有 \(O(n)\) 的时间复杂度和空间复杂度，因为递归深度与 \(n\) 成线性关系。

2. **二分递归**：例如快速排序或归并排序中的递归调用，每次都将问题分为两部分。这样的递归通常有 \(O(log n)\) 的空间复杂度（递归深度）和 \(O(n log n)\) 或 \(O(n)\) 的时间复杂度，取决于每层递归的操作量。

3. **树形递归**：如汉诺塔问题，每次递归产生多个递归调用。这种情况下，时间和空间复杂度可能会非常高，比如指数级别。
...
3. **合并**：遍历其余的区间，与结果列表中最后一个区间进行比较，看是否有重叠。
- 如果有重叠，合并它们。
...
回溯算法是一种用于找到所有（或某些）可能解决方案的算法，特别是在涉及约束满足问题（Constraint Satisfaction Problems，CSP）的情况下。回溯算法的核心思想是从问题的可能解决方案的空间树中，按照**深度优先搜索（DFS）**的策略，从根节点出发遍历解空间树。
...
数组越界发生在**当代码尝试访问数组的一个非法索引时**，即这个索引超出了数组的当前分配的范围。在大多数编程语言中，数组的索引从0开始，并且一个数组拥有固定的大小。**如果你试图访问一个超出这个范围的索引，将会发生越界错误。**
...
我们将使用栈（可以通过列表实现）来跟踪遇到的开括号。下面是具体步骤：

1. **初始化一个栈**：使用列表来模拟栈的行为。
2. **遍历字符串中的每个字符**：检查每个字符。
3. **如果是开括号（`(`, `{`, `[`）**：将其放入栈中。
4. **如果是闭括号（`)`, `}`, `]`）**：检查栈顶元素。如果栈顶元素是对应的开括号，则从栈中弹出；否则返回`False`，因为括号不匹配。
5. **遍历结束后**：如果栈为空，则返回`True`（所有括号都已匹配）；如果栈非空，则返回`False`（存在未匹配的开括号）。

下面是`isValidParentheses`函数的Python实现：
...
这个函数可以正确处理像 `"()"`, `"{}()"`, `"({})"` 的字符串（返回 `True`），也能处理像 `"]["`, `"[(])"`, `"[{]}"` 的字符串（返回 `False`）。
