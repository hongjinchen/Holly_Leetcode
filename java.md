...
4. **LinkedList**: `LinkedList` 在 Java 中是一个**双向链表**的实现，它实现了 **`List` 接口和 `Deque` 接口**，这意味着它既可以作为列表也可以作为双端队列使用。
...
5. **HashMap**: `HashMap` 存储**键值对（key-value pairs）**，允许使用键（key）来快速检索值（value）。它使用哈希表（hash table）来存储这些键值对，因此提供了常数时间的复杂度（O(1)）来插入和查找元素，前提是哈希函数将元素均匀地分布在桶中。
